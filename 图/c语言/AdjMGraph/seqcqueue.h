...
typedef struct
{
    QueueDataType queue[MaxQueueSize];
    int rear;   //队尾指针
    int front;  //队头指针
    int count;  //计数器
}SeqCQueue;
...
//入队列
int appendQueue(SeqCQueue *queue, QueueDataType x)
{
...
//出队列
int deleteQueue(SeqCQueue *queue, QueueDataType *x)
{
    if(queue->count == 0)
    {
        printf("队列为空！\n");
...
//取队头元素
int getQueue(SeqCQueue *queue, QueueDataType *x)
{
    if(queue->count == 0)
    {
        printf("队列为空！\n");
